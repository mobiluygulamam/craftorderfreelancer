...
class ContractController extends Controller
{
    ...
    public function index($slug)
    {
        ...
        if ($objUser->getGuard() == 'client') {
            ...
            $contracts = Contract::where('workspace_id', '=', $currentWorkspace->id)->where('client_id', $objUser->id)->with('projects', 'contract_type')->get();
            ...
            // Contracts Summary
            $cnt_contract = [];
            $cnt_contract['total'] = Contract::getContractSummary($currentWorkspace, $contracts);
            $cnt_contract['this_month'] = Contract::getContractSummary($currentWorkspace, $curr_month);
            $cnt_contract['this_week'] = Contract::getContractSummary($currentWorkspace, $curr_week);
            $cnt_contract['last_30days'] = Contract::getContractSummary($currentWorkspace, $last_30days);
    ...
    {
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        // $contract = Contract::with('comment.client','note.user')->find($id);
        $contract = Contract::with('comment.user', 'comment.client', 'note.user', 'note.client')->find($id);
    ...
    public function fileUpload(Request $request, $slug, $id)
    {
        ...
        $result = Utility::updateStorageLimit($company->id, $image_size);
