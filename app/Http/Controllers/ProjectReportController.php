...
class ProjectReportController extends Controller
{
    ...
    public function show(Request $request, $slug, $id)
    {
        ...
        if ($objUser && $currentWorkspace) {
            if ($objUser->getGuard() == 'client') {
                $project = Project::select('projects.*')->join('client_projects', 'projects.id', '=', 'client_projects.project_id')->where('client_projects.client_id', '=', $objUser->id)->where('projects.workspace', '=', $currentWorkspace->id)->where('projects.id', '=', $id)->with('users')->first();
            } else {
                $project = Project::select('projects.*')->join('user_projects', 'projects.id', '=', 'user_projects.project_id')->where('user_projects.user_id', '=', $objUser->id)->where('projects.workspace', '=', $currentWorkspace->id)->where('projects.id', '=', $id)->with('users')->first();
            }
        }
        if ($project) {
    ...
    //     }
    //     $arrTask['stages'] = $stages->pluck('name', 'id');
    ...
    public function getProjectChart($arrParam)
    {
        ...
        $stages = Stage::where('workspace_id', $arrParam['workspace_id'])->orderBy('order')->get();
        ...
        foreach ($arrDuration as $date => $label) {
            $data = Task::select('status', DB::raw('count(*) as total'))
                ...
                ->when(isset($arrParam['project_id']), function ($query) use ($arrParam) {
                    return $query->where('project_id', $arrParam['project_id']);
                })
                ->when(isset($arrParam['workspace_id']), function ($query) use ($arrParam) {
                    return $query->whereIn('project_id', function ($subQuery) use ($arrParam) {
                        $subQuery->select('id')
                            ->from('projects')
                            ->where('workspace', $arrParam['workspace_id']);
                    });
                })
                ->groupBy('status')
                ->pluck('total', 'status')
            ...
            foreach ($stages as $stage) {
                $arrTask[$stage->id][] = $data[$stage->id] ?? 0;
            }
            $arrTask['label'][] = __($label);
        }

        $arrTask['stages'] = $stageNames;
    ...
    public function ajax_tasks_report(Request $request, $slug, $id)
    {
        ...
        foreach ($tasks as $task) {
            ...
            $start_date = '<span class="text-body">' . date('D-M-Y', strtotime($task->start_date)) . '</span> ';

            $due_date = '<span class="text-' . ($task->due_date < date('D-M-Y') ? 'danger' : 'success') . '">' . date('D-M-Y', strtotime($task->due_date)) . '</span> ';
