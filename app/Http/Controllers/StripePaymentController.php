            ...
            if ($plan) {
                try {
                    ...
                    try {
                        ...
                        $plan_id = $plan->id;
                        $coupon_id = isset($coupons->id) ? $coupons->id : null;
                        ...
                        $return_url_parameters = function ($return_type) use ($payment_plan, $payment_type, $plan_id, $product, $price, $coupon_id) {
                            return '&return_type=' . $return_type . '&payment_processor=stripe&payment_frequency=' . $payment_plan . '&payment_type=' . $payment_type . '&plan_id=' . $plan_id
                                . '&plan_name=' . $product . '&price=' . $price . '&coupon_id=' . $coupon_id;
                        };
                        ...
                        switch ($payment_type) {
                                    ...
                                    $stripe_session = \Stripe\Checkout\Session::create(
                                        [
                                            ...
                                            'line_items' => [
                                                [
                                                    'price_data' => [
                                                        'currency' => $setting['currency'] ? $setting['currency'] : 'USD',
                            ...
                            case 'recurring':
                                ...
                                $stripe_plan_id = $plan->id . '_' . $payment_plan . '_' . $stripe_formatted_price . '_' . $setting['currency'];
                    ...
                    return redirect()->route('payment', [$request->stripe_payment_frequency, \Illuminate\Support\Facades\Crypt::encrypt($plan->id),])->with('error', __($e->getMessage()));
                }
                return redirect()->route('payment', [$request->stripe_payment_frequency, \Illuminate\Support\Facades\Crypt::encrypt($plan->id),])->with(['stripe_session' => $stripe_session]);
            } else {
                return redirect()->route('payment', [$request->stripe_payment_frequency, \Illuminate\Support\Facades\Crypt::encrypt($plan->id),])->with('error', __('Plan is deleted.'));
            }
        } else {
            return redirect()->route('payment', [$request->stripe_payment_frequency, \Illuminate\Support\Facades\Crypt::encrypt($plan->id),])->with('error', __('Plan is deleted.'));
        }
            ...
            if ($request->return_type == 'success') {
                ...
                if ($assignPlan['is_success']) {
                    $planData = [
                        ...
                        'plan_price' => Utility::getPlanActualPrice($request->plan_id, $request->payment_frequency)
                    ];
                    Utility::referraltransaction($planData);
    ...
    public function webhookStripe(Request $request)
    {
        ...
        try {
            $event = \Stripe\Webhook::constructEvent(
                $payload,
                $sig_header,
                $paymentSetting['stripe_webhook_secret']
            );
