...
class Project extends Model
{
    ...
    public function project_progress()
    {
        ...
        $getDoneStage = Stage::where('workspace_id', $project->workspace)->orderby('id', 'desc')->first();
        $total_task = Task::where('project_id', '=', $this->id)->count();
        $completed_task = Task::where('project_id', '=', $this->id)->where('status', '=', $getDoneStage->id)->count();
