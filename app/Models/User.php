...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function getUserPermissionAndLoginData()
    {
        return $this->hasOne('App\Models\UserWorkspace', 'user_id', 'id');
    }
    ...
    public static function userPlanFrequency($plan_id)
    {
        $userPlanFrequency = Order::where('plan_id', '=', $plan_id)->where('user_id', '=', \Auth::user()->id)->orderBy('updated_at', 'desc')->first();
        $frequency = isset($userPlanFrequency->payment_frequency) ? $userPlanFrequency->payment_frequency : 'monthly';
        return $frequency;
    }
...
public static function verifyEmail($pmail) {
    ...
    public function assignPlan($planID, $frequency = '')
    {
        ...
        if ($plan) {
            ...
            foreach ($workspaces as $workspace) {
                $workspacesCount++;
                $workspace->is_active = $plan->max_workspaces == -1 ? 1 : (($workspacesCount <= $plan->max_workspaces) ? 1 : 0);
                $workspace->save();
    ...
    public function cancel_subscription($user_id = false)
    {
        ...
        switch ($type) {
            ...
            case 'paypal':
                ...
                /* Initiate paypal */
                $paypal = new \PayPal\Rest\ApiContext(new \PayPal\Auth\OAuthTokenCredential($paymentSetting['paypal_client_id'], $paymentSetting['paypal_secret_key']));
    ...
    // public function notifications($workspace_id)
    // {
    //     return Notification::with('project', 'task', 'user')
        ...
        foreach ($emailTemplate as $eTemp) {
            EmailTemplate::create(
                [
                    ...
                    'from' => $setting['app_name'] ? $setting['app_name'] : "Taskly SaaS",
                ]
            );
        }
