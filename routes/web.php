...
// })->middleware(['auth'])->name('dashboard');pw
...
Route::prefix('client')->as('client.')->group(function () {
    ...
    Route::delete('/{slug}/projects/{id}/bug_comment/{tid}/file/{fid}', [ProjectController::class, 'bugDestroyFile'])->name('bug.comment.destroy.file')->middleware(['auth:client', 'XSS']);
...
Route::group(['middleware' => ['verified']], function () {
    ...
    // route for change plan status enable/disable
    Route::post('/plans/status/update', [PlanController::class, 'managePlanStatus'])->name('update.plan.status')->middleware(['auth', 'XSS']);
    ...
    // For Plan Refund
    Route::get('plan/refund/{orderId}/{userId}', [PlanController::class, 'planRefund'])->name('order.refund')->middleware(['auth', 'XSS']);
    ...
    Route::get('/users', [UserController::class, 'index'])->name('admin.users.index')->middleware(['auth', 'CheckPlan', 'XSS']);
    ...
    Route::delete('/plans/delete/{planId}', [PlanController::class, 'destroy'])->name('plans.delete')->middleware(['auth', 'XSS']);
    ...
    // bug report for company and user
    ...
    Route::get('/{slug}/projects/{id}/expenses', [ProjectController::class, 'expenseReport'])->name('projects.expense.report')->middleware(['auth', 'CheckPlan', 'XSS']);
    ...
    // Route::get('/{slug}/projects/{id}/bug_report', [ProjectController::class, 'bugReport'])->name('projects.expense.report')->middleware(['auth:client', 'XSS']);
    ...
    // yookassa Routes
    Route::any('/yookassa/payment', [YooKassaController::class, 'planPayWithYookassa'])->name('plan.pay.with.yookassa');
    Route::any('/yookassa/payment/{plan}', [YooKassaController::class, 'planGetYookassaStatus'])->name('plan.get.yookassa.status');

    // paiementpro Routes
    Route::any('/paiementpro/payment', [PaiementproController::class, 'planPayWithPaiementpro'])->name('plan.pay.with.paiementpro')->middleware(['auth', 'XSS']);
    ...
    // Nepalste Routes
    Route::any('/nepalste/payment', [NepalsteController::class, 'planPayWithNepalste'])->name('plan.pay.with.nepalste')->middleware(['auth', 'XSS']);
    Route::any('/plan-nepalste-status/', [NepalsteController::class, 'planGetNepalsteStatus'])->name('plan.nepalste.status')->middleware(['auth', 'XSS']);
    Route::get('plan-nepalste-cancel/', [NepalsteController::class, 'planGetNepalsteCancel'])->name('plan.nepalste.cancel')->middleware(['auth', 'XSS']);

    // Cinetpay Routes
    Route::any('/cinetpay/payment', [CinetPayController::class, 'planPayWithCinetpay'])->name('plan.pay.with.cinetpay')->middleware(['auth', 'XSS']);
    Route::any('plan-cinetpay-return', [CinetPayController::class, 'planCinetPayReturn'])->name('plan.cinetpay.return')->middleware(['auth', 'XSS']);
    ...
    Route::any('plan-fedapay-status', [FedapayPaymentController::class, 'planGetFedapayStatus'])->name('plan.fedapay.status');


    // payhere Routes
    Route::any('/payhere/payment', [PayHerePaymentController::class, 'planPayWithPayhere'])->name('plan.pay.with.payhere');
    Route::any('plan-payhere-status', [PayHerePaymentController::class, 'planGetPayHereStatus'])->name('plan.payhere.status');
    ...
    // Routes For Referal Program
    Route::resource('referral-programs', ReferralProgramController::class)->middleware(['auth', 'XSS']);
    Route::get('referral-program/company/{slug}', [ReferralProgramController::class, 'companyIndex'])->name('referral-program.company')->middleware(['auth', 'XSS']);
