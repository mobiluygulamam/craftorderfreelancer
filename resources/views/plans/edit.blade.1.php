    ...
    <div class="modal-body">
        <div class="row">
            ...
            @if ($plan->id != 1)
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="plan_type" class="form-label">{{ __('Plan Type') }}</label><x-required></x-required>
                        <input type="text" class="form-control" id="plan_type" name="plan_type" required  value="{{ $plan->plan_type ?? "" }}"/>
                    </div>
                </div>
                 <div class="form-group col-md-6">
                    <label for="weekly_price"
                        class="form-control-label">{{ __('Weekly Price') }}</label><x-required></x-required>
                    <div class="form-icon-user">
                        <span
                            class="currency-icon bg-primary">{{ $setting['currency_symbol'] ? $setting['currency_symbol'] : '$' }}</span>
                        <input class="form-control currency_input" type="number" min="0" id="weekly_price"
                            name="weekly_price" placeholder="{{ __('Weekly  Price') }}" step="0.01" value="{{ $plan->weekly_price }}" required >
                    </div>
               </div>
                <div class="form-group col-md-6">
