...
@once
    @php
        $chagptEnabled = $currentWorkspace->is_chagpt_enable(); // Replace with the correct namespace for your Workspace model
    @endphp
@endonce
...
@section('links')
    ...
    @if (\Auth::guard('client')->check())
        <li class="breadcrumb-item"><a
                href="{{ route($client_keyword . 'contracts.index', $currentWorkspace->slug) }}">{{ __('Contracts') }}</a>
        </li>
    @else
        <li class="breadcrumb-item"><a
                href="{{ route($client_keyword . 'contracts.index', $currentWorkspace->slug) }}">{{ __('Contracts') }}</a>
    ...
    <div class="row align-items-center text-end all-button-box justify-content-end m-1">
        @if ($chagptEnabled)
            <div class="col-auto pe-0">
                <a href="#" data-size="lg" data-ajax-popup-over="true" class="btn btn-sm btn-primary"
                    data-url="{{ route('generate', ['contract details']) }}" data-bs-toggle="tooltip"
                    data-bs-placement="top" title="{{ __('Generate with AI') }}"
                    data-title="{{ __('Generate Description, Comments & Notes') }}">
        ...
        @if ($currentWorkspace->permission == 'Owner')
            <div class="col-auto pe-0 px-2">
                <a href="{{ route($client_keyword . 'send.mail.contract', [$currentWorkspace->slug, $contract->id]) }}"
            ...
            @if ($contract->status == 'accept' || $currentWorkspace->permission == 'Owner')
                <div class="col-auto pe-0 px-2">
                    <a href="#" class="btn btn-sm btn-primary btn-icon"
                        data-url="{{ route($client_keyword . 'contracts.copy', [$currentWorkspace->slug, $contract->id]) }}"
                        data-ajax-popup="true" data-title="{{ __('Duplicate Contract') }}" data-size="lg"
                        title="{{ __('Duplicate') }}" data-bs-toggle="tooltip" data-bs-placement="top">
        ...
        @if ($currentWorkspace->permission == 'Owner' || \Auth::user()->getGuard() == 'client')
            <div class="col-auto pe-0 px-2">
                <a href="{{ route($client_keyword . 'contract.download.pdf', [$currentWorkspace->slug, \Crypt::encrypt($contract->id)]) }}"
        ...
        @if ($currentWorkspace->permission == 'Owner' || \Auth::user()->getGuard() == 'client')
            <div class="col-auto pe-0 px-2">
                <a href="{{ route($client_keyword . 'get.contract', [$currentWorkspace->slug, $contract->id]) }}"
                    target="_blank" class="btn btn-sm btn-primary btn-icon" title="{{ __('Preview') }}"
        ...
        @if (
            ($currentWorkspace->permission == 'Owner' && $contract->company_signature == null) ||
                (\Auth::user()->getGuard() == 'client' && $contract->client_signature == null))
            @if ($contract->status == 'accept' || $currentWorkspace->permission == 'Owner')
                <div class="col-auto pe-0 px-2">
                    <a href="#" class="btn btn-sm btn-primary btn-icon"
                        data-url="{{ route($client_keyword . 'signature', [$currentWorkspace->slug, $contract->id]) }}"
                        data-ajax-popup="true" data-title="{{ __('Create Signature') }}" data-size="md"
                        title="{{ __('Signature') }}" data-bs-toggle="tooltip" data-bs-placement="top">
            ...
            <div class="col-auto pe-0">
                <ul class="list-unstyled mb-0 ms-1">
                    <li class="dropdown dash-h-item drp-language">
                        <a class="dash-head-link dropdown-toggle arrow-none me-0 ms-0 p-2 rounded-1"
                            data-bs-toggle="dropdown" href="#" role="button" aria-haspopup="false"
                            aria-expanded="false">
                            <span class="drp-text hide-mob">
                                <i class=" drp-arrow nocolor hide-mob">{{ ucfirst($contract->status) }}<span
                                        class="ti ti-chevron-down px-1"></span></i>
                        </a>
                                        ...
                                        <address class="mb-0 text-sm">
                                            <dl class="row mt-4 align-items-center">
                                                ...
                                                <dd class="col-sm-8 text-sm">
                                                    {{ !empty($contract->projects) ? $contract->projects->name : '' }}</dd>
                                                ...
                                                <dd class="col-sm-8 text-sm">
                                                    {{ App\Models\Utility::dateFormat($contract->start_date) }}</dd>

                                                <dt class="col-sm-4 h6 text-sm">{{ __('End Date') }}</dt>
                                                <dd class="col-sm-8 text-sm">
                                                    {{ App\Models\Utility::dateFormat($contract->end_date) }}</dd>
                                            </dl>
                                        </address>
                                    </div>
                                </div>
                            ...
                            <div class="card-body p-3">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group mt-3">
                                        <textarea class="tox-target summernote" name="contract_description" id="summernote" rows="8">{!! $contract->contract_description !!}</textarea>
                                    </div>
                                </div>
                                            ...
                                            @if ($contract->status == 'accept' || $currentWorkspace->permission == 'Owner')
                                                <div class="col-12 d-flex">
                                                    <div class="form-group mb-0 form-send w-100">
                                                        <form method="post" class="card-comment-box needs-validation"
                                                            novalidate id="form-comment"
                                                            action="{{ route($client_keyword . 'comment_store.store', [$currentWorkspace->slug, $contract->id]) }}">
                                                            @csrf
                                                            <textarea rows="3" class="form-control grammer_textarea_comment" name="comment" placeholder="Add a comment..."
                                                                required></textarea>
                                                            ...
                                                            <grammarly-extension data-grammarly-shadow-root="true"
                                                                ...
                                                                class="cGcvT"></grammarly-extension>
                                                            <button id="" type="submit" class="btn btn-send"><i
                                                            ...
                                                            <div class="row align-items-center">
                                                                <div class="col-auto">
                                                                    <a href="#"
                                                                        ...
                                                                        <img class="avatar-sm rounded-circle img-thumbnail"
                                                                            width=""
                                                                            style="max-width: 30px; max-height: 30px;"
                                                                            @if ($comment->user_id != '' && $comment->user_id != null) @if ($comment->user->avatar) src="{{ asset('/storage/users-avatar/' . $comment->user->avatar) }}"
                                                                   @else
                                                                   avatar="{{ $comment->user->name }}" @endif
                                                                        alt="{{ $comment->user->name }}" @else
                                                                            @if ($comment->client->avatar) src="{{ asset('/storage/users-avatar/' . $comment->client->avatar) }}"
                                                                   @else
                                                                   avatar="{{ $comment->client->name }}" @endif
                                                                ...
                                                                @if (\Auth::guard('client')->check())
                                                                    ...
                                                                    @if ($contract->status == 'accept' && \Auth::user()->id == $comment->client_id)
                                                                        <div class="col-auto">
                                                                            <a href="{{ route($client_keyword . 'comment_store.destroy', [$currentWorkspace->slug, $comment->id]) }}"
                                                                                class="action-btn btn-danger mx-1 mt-1 btn btn-sm d-inline-flex align-items-center"
                                                                                title="{{ __('Delete') }}">
                                    ...
                                    <div class="card">
                                                ...
                                                @if ($chagptEnabled)
                                                    <div class="col-6 text-end">
                                                        <a data-size="md"
                                                            class="btn btn-primary btn-icon btn-sm text-white"
                                                            data-ajax-popup-over="true" id="grammarCheck"
                                                            data-url="{{ route('grammar', ['contract_note']) }}"
                                                            data-bs-placement="top"
                                                            data-title="{{ __('Grammar check with AI') }}">
                                                            <i class="ti ti-rotate"></i>
                                                            <span>{{ __('Grammar check with AI') }}</span></a>
                                        ...
                                        <div class="card-body">
                                            @if ($contract->status == 'accept' || $currentWorkspace->permission == 'Owner')
                                                <div class="col-12 d-flex">
                                                    <div class="form-group mb-0 form-send w-100">
                                                        <form method="post" class="card-note-box needs-validation"
                                                            novalidate id="form-note"
                                                            action="{{ route($client_keyword . 'note_store.store', [$currentWorkspace->slug, $contract->id]) }}">
                                                            @csrf
                                                            <textarea rows="3" class="form-control grammer_textarea_note" name="notes" data-toggle="autosize"
                                                                placeholder="Add a note..." spellcheck="false" required></textarea>
                                                            <grammarly-extension data-grammarly-shadow-root="true"
                                                                style="position: absolute; top: 0px; left: 0px; pointer-events: none; z-index: 1;"
                                                                class="cGcvT"></grammarly-extension>
                                                            <grammarly-extension data-grammarly-shadow-root="true"
                                                        ...
                                                        <div class="row align-items-center">
                                                            <div class="col-auto">
                                                                <a href="#"
                                                                    ...
                                                                    <img class="avatar-sm rounded-circle img-thumbnail"
                                                                        width=""
                                                                        style="max-width: 30px; max-height: 30px;"
                                                                        @if ($note->user_id != '' && $note->user_id != null) @if ($note->user->avatar) src="{{ asset('/storage/users-avatar/' . $note->user->avatar) }}"
                                                                   @else
                                                                   avatar="{{ $note->user->name }}" @endif
                                                                    alt="{{ $note->user->name }}" @else
                                                                        @if ($note->client->avatar) src="{{ asset('/storage/users-avatar/' . $note->client->avatar) }}"
                                                                   @else
                                                                   avatar="{{ $note->client->name }}" @endif
                                                                        alt="{{ $note->client->name }}" @endif />
                                                                </a>
                                                            </div>
                                                            <div class="col ml-n2">
                                                                <p
                                                            ...
                                                            @if (\Auth::guard('client')->check())
                                                                @if ($contract->status == 'accept' && \Auth::user()->id == $note->client_id)
                                                                    <div class="col-auto">
                                                                        <a href="{{ route($client_keyword . 'note_store.destroy', [$currentWorkspace->slug, $note->id]) }}"
                                                                            class="action-btn btn-danger mx-1 mt-1 btn btn-sm d-inline-flex align-items-center"
                                                                            title="{{ __('Delete') }}">
                                                                            <span class="text-white">
                                                                                <i class="ti ti-trash"></i></span>
        ...
        myDropzone = new Dropzone("#dropzonewidget", {
            ...
            success: function(file, response) {
                ...
                if (response.is_success) {
                    ...
                    show_toastr('{{ __('Success') }}', 'File Successfully Uploaded' + '</br>' +
                        '<span class="text-danger">' + response.result + '</span>', 'success');
                } else {
