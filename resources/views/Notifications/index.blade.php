                    ...
                    @if ($currentWorkspace->is_chagpt_enable())
                        <ul class="list-unstyled mb-0 m-3">
                            <a href="#" data-size="lg" data-ajax-popup-over="true" class="btn btn-sm btn-primary"
                                data-url="{{ route('generate', ['notification template', $notification_template->id]) }}"
                    ...
                    <ul class="list-unstyled mb-0 m-2">
                        <li class="dropdown dash-h-item drp-language">
                            <a class="dash-head-link dropdown-toggle arrow-none me-0" data-bs-toggle="dropdown"
                                ...
                                {{-- @dump($curr_noti_tempLang->lang); --}}
                            </a>
                            <div class="dropdown-menu dash-h-dropdown dropdown-menu-end" aria-labelledby="dropdownLanguage">
                                @foreach ($languages as $langCode => $lang)
                                    <a href="{{ route('notification-templates.index', [$currentWorkspace->slug, $notification_template->id, $langCode]) }}"
                                        class="dropdown-item {{ $curr_noti_tempLang->lang == $langCode ? 'text-primary' : '' }}">{{ ucfirst(\App\Models\Utility::getlang_fullname($langCode)) }}</a>
                                @endforeach
                            </div>
                        </li>
                    </ul>
...
@section('links')
    ...
    <li class="breadcrumb-item"> <a
            href="{{ route('notification-templates.index', $currentWorkspace->slug) }}">{{ __('Notifications') }}</a></li>
