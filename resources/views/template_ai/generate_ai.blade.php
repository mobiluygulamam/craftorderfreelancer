...
<form action="" id="myForm" class="needs-validation" novalidate>
    ...
    <div class="response row col-sm-12">
        <a class="btn btn-primary btn-sm col-md-2 col-12 my-2 mx-2 " href="#!"
            id="generate">{{ __('Generate') }}</a>
        <a href="#!" onclick="copyText()" class="btn btn-primary btn-sm col-12 col-md-2 my-2 mx-2 float-end "><i
                class="ti ti-copy"></i>{{ __('Copy Text') }}</a>
        <a href="#!" onclick="copySelectedText()"
            class="btn btn-primary btn-sm col-12 col-md-3 my-2 mx-2 float-end me-2"><i
...
<script>
    function copyText() {
        ...
        if (copied.length <= 0) {
            show_toastr('error', 'Description Text Is Not Copied', 'error');
        } else {
            var input = $('input[name=' + selected + ']').length;
            if (input > 0) {
                var data = $('input[name=' + selected + ']').val(copied)
            } else {
    ...
    function copySelectedText() {
        ...
        if (selectedText.length <= 0) {
            show_toastr('error', 'Selected Text Is Not Copied', 'error');
        } else {
            var input = $('input[name=' + selected + ']').length;
