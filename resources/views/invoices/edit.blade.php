        ...
        <div class="row">
            <div class="form-group col-md-6">
                ...
                <select class="form-control" name="status" id="status" required>
                    <option value="sent" @if ($invoice->status == 'sent') selected @endif>{{ __('Sent') }}
                    </option>
                    <option value="paid" @if ($invoice->status == 'paid') selected @endif>{{ __('Paid') }}
                    </option>
                    <option value="partialy paid" @if ($invoice->status == 'partialy paid') selected @endif>
                        {{ __('Partialy Paid') }}</option>
                    <option value="canceled" @if ($invoice->status == 'canceled') selected @endif>{{ __('Canceled') }}
                    </option>
                </select>
            </div>
            <div class="form-group col-md-6">
            ...
            <div class="form-group col-md-6">
                ...
                <div class="input-group date ">
                    <input class="form-control datepicker3" type="text" id="issue_date" name="issue_date"
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control" name="tax_id" id="tax_id">
                    ...
                    @foreach ($taxes as $p)
                        <option value="{{ $p->id }}" @if ($invoice->tax_id == $p->id) selected @endif>
                            {{ $p->name }}</option>
                    @endforeach
                </select>
            </div>
