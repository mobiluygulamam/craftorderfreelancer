                         ...
                         <tbody>
                             @foreach ($orders as $order)
                                 ...
                                 <tr>
                                     ...
                                     <td>{{ (!empty($setting['currency_symbol']) ? $setting['currency_symbol'] : '$') . $order->price }}</td>
                                     ...
                                     @if (Auth::user()->type == 'admin')
                                         ...
                                         <td>
                                             @foreach ($userOrders as $userOrder)
                                                 @if ($user->plan == $order->plan_id && $order->order_id == $userOrder->order_id && $order->is_refund == 0 && $order->payment_type !== 'Manually Upgrade By Super Admin')
                                                     <div class="badge bg-warning rounded p-2 px-3 ms-2">
                                                         <a href="{{ route('order.refund', [$order->id, $order->user_id]) }}" class="mx-3 align-items-center" data-bs-toggle="tooltip" title="{{ __('Delete') }}" data-original-title="{{ __('Delete') }}">
