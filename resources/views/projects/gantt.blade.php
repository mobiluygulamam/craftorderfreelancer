...
@section('action-button')
    <div class="btn-group mx-1" id="change_view" role="group">
        <a href="{{ route($is_client . 'projects.gantt', [$currentWorkspace->slug, $project->id, 'Quarter Day']) }}"
            class="btn btn-sm px-3 btn-info @if ($duration == 'Quarter Day') active @endif"
            data-value="Quarter Day">{{ __('Quarter Day') }}</a>
        <a href="{{ route($is_client . 'projects.gantt', [$currentWorkspace->slug, $project->id, 'Half Day']) }}"
            class="btn btn-sm  px-3 btn-info @if ($duration == 'Half Day') active @endif"
            data-value="Half Day">{{ __('Half Day') }}</a>
        <a href="{{ route($is_client . 'projects.gantt', [$currentWorkspace->slug, $project->id, 'Day']) }}"
            class="btn btn-sm px-3 btn-info @if ($duration == 'Day') active @endif"
            data-value="Day">{{ __('Day') }}</a>
        <a href="{{ route($is_client . 'projects.gantt', [$currentWorkspace->slug, $project->id, 'Week']) }}"
            class="btn btn-sm px-3 btn-info @if ($duration == 'Week') active @endif"
            data-value="Week">{{ __('Week') }}</a>
        <a href="{{ route($is_client . 'projects.gantt', [$currentWorkspace->slug, $project->id, 'Month']) }}"
            class="btn btn-sm px-3 btn-info @if ($duration == 'Month') active @endif"
            data-value="Month">{{ __('Month') }}</a>
        @auth('client')
            <a href="{{ route('client.projects.show', [$currentWorkspace->slug, $project->id]) }}"
                class="btn-submit btn btn-sm btn-primary btn-icon" data-toggle="tooltip" title="{{ __('Back') }}">
...
@if ($project && $currentWorkspace)
    @push('css-page')
        <link rel="stylesheet" href="{{ asset('assets/custom/css/frappe-gantt.css') }}" />
    @endpush
    @push('scripts')
        @php
