            ...
            <form method="POST" action="{{ route('users_logs.filter', $currentWorkspace->slug) }}" class="row">
                ...
                <div class="form-group col-md-2 col-sm-2">
                    <div class="input-group date ">
                        <input class="form-control datepicker5" type="text" id="start_date" name="start_date"
                            value="" autocomplete="off" placeholder="{{ __('Start Date') }}">
                ...
                <div class=" col-md-2 col-sm-2 pb-2">
                    <a href="{{ route('users_logs.index', $currentWorkspace->slug) }}" class="btn btn-md btn-danger"
                                    ...
                                    @foreach ($user_logs as $key => $user_log)
                                        ...
                                        @if ($current_month == $month && $current_year == $year)
                                            <tr>
                                                ...
                                                <td><span class="badge p-2 px-3 rounded bg-success">
                                                        {{ $user_log->role }}</span>
                                                </td>
                                                <td>{{ $user_log->date }}</td>
                                                <td>{{ $user_log->ip }}</td>
                                                ...
                                                @auth('web')
                                                    <td class="text-right">
                                                        <a href="#"
                                                            ...
                                                            data-url="{{ route('users_logs.show', [$currentWorkspace->slug, $user_log->id]) }}"
                                                            data-toggle="tooltip" title="{{ __('User Log') }}">
                                                            <i class="ti ti-eye"></i>
                                                        </a>
                                                        <a href="#"
                                                            ...
                                                            data-confirm-yes="delete-form-{{ $user_log->id }}"
                                                            data-toggle="tooltip" title="{{ __('Delete User Log') }}">
                                                            <i class="ti ti-trash"></i>
                                                        </a>
                                                        {!! Form::open([
                                                            'method' => 'DELETE',
                                                            'route' => ['users_logs.destroy', [$currentWorkspace->slug, $user_log->id]],
                                                            'id' => 'delete-form-' . $user_log->id,
                                                        ]) !!}
                                                        {!! Form::close() !!}
                                                    </td>
                                                @endauth
